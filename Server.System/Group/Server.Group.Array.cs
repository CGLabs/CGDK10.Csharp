...
namespace CGDK.Server.Group
{
	...
	// constructor/destructor) 
		...
		{
			// - dispatchable
			m_component_MessageTransmitter			 = new MessageTransmitter();

			// - State
			m_component_state = new ObjectState
			{
				Target = this,
	...
	// publics) 
		public eOBJECT_STATE		Now
		{
			get
			{
				return m_component_state.Now;
			}
			set
			{
				m_component_state.Now = value;
			}
		}
	...
	// frameworks)
		...
		public int					ProcessMessage (object _source, sMESSAGE _msg)
		{
			...
			if(m_component_MessageTransmitter != null)
			{
				var result = m_component_MessageTransmitter.TransmitMessage(_source, _msg);
