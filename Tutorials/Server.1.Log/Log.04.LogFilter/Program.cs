...
namespace tutorial.server.log._04.log_filter
{
	class Program
	{
		static void Main()
		{
			...
			//       - EnableLog() 멤버 함수를 사용하여 특정 [로그 형태]의 필터링 가능 여부를 설정한다.
			//       - EnableLevelRange()를 사용하여 레벨 정보로 필터링할 것임을 설정해야 한다.
			//       - SetLevelRange()함수를 사용하여 통과할 레벨 정보를 설정해야 한다.
			//
			//       주의) 필터링 정보는 Level, Source, Destination, Location 등이 있지만 현재는 Level만 사용한다.
			//
			//    5) SetLevelRange()함수를 사용해 필터링 레벨 범위를 설정할 수 있다.
			...
			// ------------------------------------------------------------------
			// 1) Log Filter를 생성한다.
			var plog_filter = new CGDK.Server.LogFilter();
			...
			plog_filter.EnableLog			(eLOG_TYPE.INFO);
			plog_filter.EnableLevelRange	(eLOG_TYPE.INFO);
			plog_filter.SetLevelRange		(eLOG_TYPE.INFO, eLOG_LEVEL.HIGHER, eLOG_LEVEL.HIGHEST);

			// 3) PROGRESS Log type에 대해서 filtering을 enable한다.
			//    PROGRESS Log type은 eLOG_LEVEL::NORMAL이상 eLOG_LEVEL::HIGHEST이하일 때만 출력한다.
			plog_filter.EnableLog			(eLOG_TYPE.PROGRESS);
			plog_filter.EnableLevelRange	(eLOG_TYPE.PROGRESS);
			plog_filter.SetLevelRange		(eLOG_TYPE.PROGRESS, eLOG_LEVEL.NORMAL, eLOG_LEVEL.HIGHEST);

			// 4) Exception Log는 enable합니다.
			plog_filter.EnableLog(eLOG_TYPE.DEBUG,		false);
			plog_filter.EnableLog(eLOG_TYPE.EXCEPTION,	true);
			plog_filter.EnableLog(eLOG_TYPE.ERROR,		true);
			plog_filter.EnableLog(eLOG_TYPE.WARNING,	false);
			plog_filter.EnableLog(eLOG_TYPE.USER,		false);
			plog_filter.EnableLog(eLOG_TYPE.SYSTEM,	true);
			...
			// ------------------------------------------------------------------
			// 1) log_console 생성한다.
			var plog_console = new CGDK.Server.LoggerConsole
			{
				// - log filter를 log객체에 설정한다.
				Filter = plog_filter
			};
