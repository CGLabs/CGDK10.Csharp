...
namespace tutorial.time_event._01.basic.console
{
	class Program
	{
		static void Main()
		{
			...
			// ------------------------------------------------------------------
			// 1. CGDK.Server.TimeEvent.entity_iteration
			//
			...
			var pentity_a = new CGDK.Server.TimeEvent.EntityIteration();

			// 2) setting
			pentity_a.EntityStatus.timeNext = DateTime.UtcNow + TimeSpan.FromSeconds(1);
			...
			// ------------------------------------------------------------------
			// 1) time_event 객체를 생성한다.
			var pentity_b = new CGDK.Server.TimeEvent.EntityStartEnd
			{
				// 2) setting
				StartTime = DateTime.UtcNow + TimeSpan.FromSeconds(5),
				EndTime = DateTime.UtcNow + TimeSpan.FromSeconds(15)
			};
			...
			// ------------------------------------------------------------------
			// 3. time_event::EntityStartIterationEnd
			//
			//    - 시작 시간과 process_event_Start()를 호출하고
			...
			// ------------------------------------------------------------------
			// 1) time_event 객체를 생성한다.
			var pentity_c = new CGDK.Server.TimeEvent.EntityStartIterationEnd
			{
				// 2) setting
				StartTime = DateTime.UtcNow + TimeSpan.FromSeconds(10),
				interval = TimeSpan.FromSeconds(5),
				EndTime = DateTime.UtcNow + TimeSpan.FromSeconds(1)
			};
