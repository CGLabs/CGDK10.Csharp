	...
	class Program
	{
		static void Main()
		{
			...
			// ------------------------------------------------------------------
			{
				...
				var pentity = new CGDK.Server.TimeEvent.Entity ("test_event_1");

				// 2) make event - Start
				{
					// - event setting
					var event_setting = new CGDK.Server.TimeEvent.sEVENT_SETTING
					{
						Name = "Start",
						Type = CGDK.Server.TimeEvent.eTYPE.NONE,
					...
					var event_object = new CGDK.Server.TimeEvent.NEvent (event_setting, (DateTime _time_now) =>
					{
					...
					pentity.RegisterEvent(event_object);
				}
				...
				// 3) make event - end
				{
					...
					var event_setting = new CGDK.Server.TimeEvent.sEVENT_SETTING
					{
						Name = "end",
						Type = CGDK.Server.TimeEvent.eTYPE.NONE,
						timeExecute = DateTime.UtcNow + TimeSpan.FromSeconds(5),
						timeInterval = TimeSpan.Zero,
						countTimes = 1
					};
			...
			{
				// 1) create event entity
				var pentity = new CGDK.Server.TimeEvent.Entity("test_event_2");

				// 2) 시작한 후 3초 실행하는 event 객체를 추가한다.
				pentity.RegisterEvent(CGDK.Server.TimeEvent.Entity.MakeEventOnce(DateTime.UtcNow + TimeSpan.FromSeconds(3), (DateTime _time_now) =>
				{
				...
				pentity.RegisterEvent(CGDK.Server.TimeEvent.Entity.MakeEventIteration(DateTime.UtcNow, TimeSpan.FromSeconds(5), 5, (DateTime _time_now) =>
				{
